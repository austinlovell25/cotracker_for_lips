...
def find_sync(fps, left_video, right_video):
    ...
    if right_frame <= left_frame:
        ...
        str = f"ffmpeg -ss {second_diff} -i {left_video} -c:v copy -c:a copy {left_folder}/left_sync_audio.mp4"
        print(str)
        subprocess.run(str, shell=True)
        str = f"mv {right_video} {right_folder}/right_sync_audio.mp4"
        subprocess.run(str, shell=True)
        ...
        str = f"ffmpeg -ss {second_diff} -i {right_video} -c:v copy -c:a copy {right_folder}/right_sync_audio.mp4"
        print(str)
        subprocess.run(str, shell=True)
        str = f"mv {left_video} {left_folder}/left_sync_audio.mp4"
        subprocess.run(str, shell=True)
