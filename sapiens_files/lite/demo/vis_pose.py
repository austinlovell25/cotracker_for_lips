...
def img_save_and_vis(
    ...
    # Select only specific keypoints
    #selected_ids = [73,74,75,76,77, 79, 80, 81, 82, 83] ## COCO VALUES
    #selected_ids = [item - 1 for item in selected_ids]
    # selected_ids = [190, 191, 192, 193, 194, 195, 196, 197, 200, 201, ## LIP COORDINATES
    #                 178, 179, 180, 181, 182, 183, 184, 185, 186, 187]
    #selected_ids = [190,191] # Just the two keypoints center upper and lower lip

    selected_ids = [193, 195, 196, 197, 198, 200, 201, 202, 191, 190] + [192, 194, 199, 203] + list(range(178,188)) + list(range(71,96))
    ...
    for i in range(len(heatmap)):
        ...
        selected_keypoints = (keypoints[0][selected_ids] / input_shape) * scales[i] + centres[i] - 0.5 * scales[i]
        selected_scores = keypoint_scores[0][selected_ids]

#        selected_keypoints = (keypoints[0][selected_ids] / input_shape) * scales[i] + centres[i] - 0.5 * scales[i]

        # Append only the selected info
        instance_info.append({
            "keypoints": selected_keypoints.tolist(),
            "keypoint_scores": selected_scores.tolist()
        })
        ...
        # # print(original_index[selected_ids])
        # # print(selected_index[selected_ids])
        #
        # # print(kpts[original_index])
        # x1.append(selected_keypoints[0][0])
        # y1.append(selected_keypoints[0][1])
        # x2.append(selected_keypoints[1][0])
        # y2.append(selected_keypoints[1][1])
        #
        #
        # print("------------appended------------------")
        # print(list(k))

        # For drawing, you can still use the full set if needed
    ...
    for kpts, score, visible in zip(
        ...
        ## UPDATED VERSION FOR KIM LAB
        for selected_index, original_index in enumerate(selected_ids): ### This iterates through my selected points but keeps color index
            kpt = kpts[original_index]
            kpt_score = score[original_index]
            if kpt_score < kpt_thr or not visible[selected_index] or kpt_colors[original_index] is None: ### may need to include visible
                continue

            color = kpt_colors[original_index]
            ...
            ### TESTING PLOTTING (JAMES)
            #img = cv2.circle(img, (400, 400), 500, (255, 0, 0), -1)
...
#df = pd.DataFrame(list(zip(x1, y1, x2, y2)), columns=['x1', 'y1', 'x2', 'y2'])
#df.to_csv('/home/kwangkim/Desktop/sapiens/pose/Outputs/vis/itw_videos/TrainingSet.csv')
