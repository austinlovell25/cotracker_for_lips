...
if is_mult == "False":
    ...
    if os.path.splitext(file_path)[1] == ".json":
        with open(file_path, 'r') as file:
            ...
            ### not adding crop shifting cuz it isn't implemented for sapiens
            times = data["times"]
            for time in times:
                ...
                for key, value in configs.items():
                    str = f"bash sapiens_cotracker.sh {vid1} {vid2} {exp_name}_{time}_{key} {value} {save_dir} {cam_config_dir} {is_snap} {file.name} {is_cotracker_three}"
                    subprocess.run(str, shell=True)
