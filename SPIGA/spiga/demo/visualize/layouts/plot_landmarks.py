...
class LandmarkLayout(BasicLayout):
    ...
    def draw_landmarks(self, image, landmarks, visible=None, mask=None,
        ...
        # Plot landmarks
        canvas = self.draw_circles(image, ldm_vis[0: 48], color = colors[0], thick=thick)
        canvas = self.draw_circles(image, ldm_vis[27: 28], color=BasicLayout.colors['green'], thick=thick)
        canvas = self.draw_circles(image, ldm_vis[48: 68], color=BasicLayout.colors['green'], thick=thick)
        canvas = self.draw_circles(image, ldm_vis[57: 58], color=BasicLayout.colors['green'], thick=thick)
        canvas = self.draw_circles(image, ldm_vis[51: 52], color=BasicLayout.colors['green'], thick=thick)
        canvas = self.draw_circles(image, ldm_vis[39: 40], color=BasicLayout.colors['green'], thick=thick)
        canvas = self.draw_circles(image, ldm_vis[42: 43], color=BasicLayout.colors['green'], thick=thick)
