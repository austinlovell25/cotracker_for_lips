...
class SPIGAProcessor(pr.Processor):
    ...
    def process_frame(self, frame, tracked_obj, count, processorNumber):
        bboxes = []
        for obj in tracked_obj:
            x1, y1, x2, y2 = obj.bbox[:4]
            bbox_wh = [x1, y1, x2-x1, y2-y1]
            bboxes.append(bbox_wh)
        features, pose = self.processor.inference(frame, bboxes, count, processorNumber)
