...
class VideoAnalyzer:
    ...
    def process_frame(self, image, count, processorNumber):
        image = copy.copy(image)
        self.tracked_obj = self.tracker.process_frame(image, self.tracked_obj)
        if len(self.tracked_obj) > 0:
            self.tracked_obj, pose = self.processor.process_frame(image, self.tracked_obj, count, processorNumber)
        self.tracked_obj = self._add_attributes()
        return self.tracked_obj, pose
