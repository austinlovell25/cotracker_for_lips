...
class SPIGA(nn.Module):
    ...
    def forward(self, data, count, processorNumber):
        # Inputs: Visual features and points projections
        pts_proj, features, pose = self.backbone_forward(data, count, processorNumber)
        # Visual field
    ...
    def backbone_forward(self, data, count, processorNumber):
        ...
        #    pose_save = torch.load('Poses.pt')[count]
        #else:
        #    pose_save = torch.load('Poses2.pt')[count]
        features['Pose'] = pose.clone()
        #pose_save = pose.clone()
        # Project model 3D
        euler = pose[:, 0:3]
        trl = pose[:, 3:]
    ...
    def extract_visual_embedded(self, pts_proj, receptive_field, step):
        ...
        grid = torch.nn.functional.affine_grid(theta, (B * L, C, self.kwindow, self.kwindow), align_corners=True)
