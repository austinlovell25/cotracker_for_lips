...
class SPIGAFramework:
    ...
    def inference(self, image, bboxes, count, processorNumber):
        """
        @param self:
        ...
        outputs, pose = self.net_forward(batch_crops, count, processorNumber)
    ...
    def net_forward(self, inputs, count, processerNumber):
        outputs, pose = self.model(inputs, count, processerNumber)
        return outputs, pose
