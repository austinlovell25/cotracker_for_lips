...
#  rm -f 2d_lip_coordinates.csv
  ...
  for z in {0..4}
  do
    echo "$z" > SPIGA/spiga/demo/analyze/track/retinasort/shake_opt.txt
    python SPIGA/spiga/demo/app_2d.py -i "$fname1" -d 300wprivate --shake True
  done
  ...
  do
    echo "$z" > SPIGA/spiga/demo/analyze/track/retinasort/shake_opt.txt
    python SPIGA/spiga/demo/app_2d.py -i "$fname2" -d 300wprivate --shake True
  done
  ...
  # Reset shake_opt.txt
  echo 0 > SPIGA/spiga/demo/analyze/track/retinasort/shake_opt.txt
...
ffmpeg -hide_banner -nostats -loglevel 0 -i "$fname1" -y -filter:v "crop=704:512:${pts[0]}:${pts[1]}" tmp/vid1_crop.mp4
ffmpeg -hide_banner -nostats -loglevel 0 -i "$fname2" -y -filter:v "crop=704:512:${pts[2]}:${pts[3]}" tmp/vid2_crop.mp4

# Run cotracker on first video
#deactivate
#source venv/bin/activate
python quickstart.py -v tmp/vid1_crop.mp4 -n 0 -e "$exp_name" -gc "$grid_config" -d "$save_dir" --snap_middle "$is_snap" --is_cotracker_three "$is_cotracker_three"

# Run cotracker on second video
python quickstart.py -v tmp/vid2_crop.mp4 -n 1 -e "$exp_name" -gc "$grid_config" -d "$save_dir" --snap_middle "$is_snap" --is_cotracker_three "$is_cotracker_three"
